...
def load_safetensors_with_lora_and_fp8(
    ...
    if lora_weights_list is None or len(lora_weights_list) == 0:
        ...
        def weight_hook_func(model_weight_key, model_weight: torch.Tensor, keep_on_calc_device=False, is_language=is_lang):
            nonlocal list_of_lora_weight_keys, lora_weights_list, lora_multipliers, calc_device
            # print(model_weight_key)
            if not model_weight_key.endswith(".weight"):
                # print("model_weight_key not endswith .weight")
                # print(model_weight_key)
                return model_weight
            ...
            for lora_weight_keys, lora_sd, multiplier in zip(list_of_lora_weight_keys, lora_weights_list, lora_multipliers):
                ...
                if is_language:
                    lora_name = "lora_unet_model_language_" + lora_name.replace(".", "_")
                else:
                    lora_name = "lora_unet_" + lora_name.replace(".", "_")
                down_key = lora_name + ".lora_down.0.weight"
                up_key = lora_name + ".lora_up.0.weight" # 因为之前尝试过双lora，这里多了个.0
