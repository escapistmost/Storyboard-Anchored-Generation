...
# XXX 现在的工作想法是修改LoRAModule，init加一个参数，如果有这个参数，就按照输入数值设计对应数量的lora_up和lora_down,以及对应的一个切换参数，切换参数指向几，就用哪个lora进行生成
...
class LoRAModule(torch.nn.Module):
    ...
    def __init__(
        ...
        # 默认只是用第一个lora，需要使用第二个或更多的时候需要进行修改
        self.lora_id = 0
        self.lora_sum = lora_num  # lora的数量
        if split_dims is None:
            if org_module.__class__.__name__ == "Conv2d":
                kernel_size = org_module.kernel_size
                stride = org_module.stride
                padding = org_module.padding
                # XXX 为了支持多个lora并行，将原本的lora内容变成ModuleList，通过self.lora_id来指定使用哪个lora
                # self.lora_down = torch.nn.Conv2d(in_dim, self.lora_dim, kernel_size, stride, padding, bias=False)
                # self.lora_up = torch.nn.Conv2d(self.lora_dim, out_dim, (1, 1), (1, 1), bias=False)
                self.lora_down = torch.nn.ModuleList(
                    [torch.nn.Conv2d(in_dim, self.lora_dim, kernel_size, stride, padding, bias=False) for _ in range(lora_num)]
                )
                self.lora_up = torch.nn.ModuleList(
                    [torch.nn.Conv2d(self.lora_dim, out_dim, (1, 1), (1, 1), bias=False) for _ in range(lora_num)]
                )
            else:
                # XXX 为了支持多个lora并行，将原本的lora内容变成ModuleList，通过self.lora_id来指定使用哪个lora
                self.lora_down=torch.nn.ModuleList(
                    [torch.nn.Linear(in_dim, self.lora_dim, bias=False) for _ in range(lora_num)]
                )
                self.lora_up=torch.nn.ModuleList(
                    [torch.nn.Linear(self.lora_dim, out_dim, bias=False) for _ in range(lora_num)]
                )
            ...
            # )
            self.lora_up = torch.nn.ModuleList(
                [torch.nn.ModuleList([torch.nn.Linear(self.lora_dim, split_dim, bias=False) for split_dim in split_dims])
                for _ in range(lora_num)]
            )
            # self.lora_up = torch.nn.ModuleList([torch.nn.Linear(self.lora_dim, split_dim, bias=False) for split_dim in split_dims])
            # XXX 对初始化进行修改,原本只有一个循环
            for lora_downs in self.lora_down:
                for lora_down in lora_downs:
                    torch.nn.init.kaiming_uniform_(lora_down.weight, a=math.sqrt(5))
                # torch.nn.init.kaiming_uniform_(lora_downs.weight, a=math.sqrt(5))
            for lora_ups in self.lora_up:
                for lora_up in lora_ups:
                    torch.nn.init.zeros_(lora_up.weight)
                # torch.nn.init.zeros_(lora_up.weight)
    ...
    def forward(self, x):
        if self.lora_id >= self.lora_sum:
            # XXX 加一个警告，防止有人给id设置超了
            raise ValueError(f"lora_id {self.lora_id} is out of range (lora_sum: {self.lora_sum})")
        ...
        if self.split_dims is None:
            # XXX 使用self.lora_id来指定使用哪个lora
            lx = self.lora_down[self.lora_id](x)
            ...
            if self.rank_dropout is not None and self.training:
                ...
                scale = self.scale
            # XXX 使用self.lora_id来指定使用哪个lora
            lx = self.lora_up[self.lora_id](lx)

            return org_forwarded + lx * self.multiplier * scale
        else:
            # XXX 使用self.lora_id来指定使用哪个lora
            lxs = [lora_down(x) for lora_down in self.lora_down[self.lora_id]]
            ...
            if self.rank_dropout is not None and self.training:
                ...
                scale = self.scale
            # XXX 使用self.lora_id来指定使用哪个lora
            lxs = [lora_up(lx) for lora_up, lx in zip(self.lora_up[self.lora_id], lxs)]
...
def create_arch_network(
    ...
    **kwargs,
    ...
    return create_network(
        ...
        text_encoder_lora_num=text_encoder_lora_num,
        **kwargs,
    )
...
def create_network(
    ...
    **kwargs,
    ...
    # print('escapist_include_patterns',include_patterns)
    ...
    network = LoRANetwork(
        ...
        verbose=verbose,
        text_encoder_lora_num=text_encoder_lora_num,
        unet_lora_num=unet_lora_num# XXX 设置text encoder的lora数量
    )
...
class LoRANetwork(torch.nn.Module):
    ...
    # XXX 需要给他加上text encoders的部分 原本是没有的
    def __init__(
        self,
        target_replace_modules: List[str],
        prefix: str,
        ...
        logger.info(f"lora num for text_encoder : {text_encoder_lora_num} ")
        logger.info(f"lora num for unet : {unet_lora_num} ")
                    ...
                    for child_name, child_module in module.named_modules():
                        ...
                        if is_linear or is_conv2d:
                            ...
                            lora = module_class(
                                ...
                                rank_dropout=rank_dropout,
                                module_dropout=module_dropout,
                                lora_num=lora_num, # XXX 新增lora_num参数
                            )
                            loras.append(lora)
        ...
        # XXX 添加了loras_num
        self.text_encoder_lora_num=text_encoder_lora_num
        # XXX 添加了text_encoder_loras的部分 原本是[],这里直接特殊化处理了，只lora第一个（毕竟我们也只训练一个）
        self.text_encoder_loras: List[Union[LoRAModule, LoRAInfModule]]  
        # XXX 添加参数lora_num
        self.text_encoder_loras,skipped_te=create_modules(True, prefix, text_encoders[0], target_replace_modules,lora_num=self.text_encoder_lora_num)

        # create LoRA for U-Net
        self.unet_lora_num=unet_lora_num
        self.unet_loras: List[Union[LoRAModule, LoRAInfModule]]
        self.unet_loras, skipped_un = create_modules(True, prefix, unet, target_replace_modules,lora_num=unet_lora_num)
        logger.info(f"create LoRA for U-Net/DiT: {len(self.unet_loras)} modules.")
        if verbose:
            for lora in self.unet_loras:
                logger.info(f"\t{lora.lora_name:50} {lora.lora_dim}, {lora.alpha}")

        skipped = skipped_un+skipped_te
    ...
    def set_lora_use(self,if_use=True):
        logger.info(f"set lora to {str(if_use)}")
        for lora in self.text_encoder_loras + self.unet_loras:
            lora.use_lora=if_use
    
    def set_text_encoder_lora_id(self, lora_id: int):
        # XXX 设置text encoder的lora id
        if lora_id < 0 or lora_id >= self.text_encoder_lora_num:
            raise ValueError(f"lora_id {lora_id} is out of range (lora_num: {self.text_encoder_lora_num})")
        for lora in self.text_encoder_loras:
            lora.lora_id = lora_id
        # XXX 输出日志,后续要删除，因为每个inference都调用这个函数
        logger.info(f"set text encoder LoRA id to {lora_id}")
    
    def set_unet_lora_id(self, lora_id: int):
        # XXX 设置text encoder的lora id
        if lora_id < 0 or lora_id >= self.unet_lora_num:
            raise ValueError(f"lora_id {lora_id} is out of range (lora_num: {self.unet_lora_num})")
        for lora in self.unet_loras:
            lora.lora_id = lora_id
        # XXX 输出日志,后续要删除，因为每个inference都调用这个函数
        logger.info(f"set unet encoder LoRA id to {lora_id}")
    ...
    def prepare_optimizer_params(self, unet_lr: float = 1e-4, **kwargs):
        ...
        if self.text_encoder_loras:
            params, descriptions = assemble_params(self.text_encoder_loras, unet_lr, self.loraplus_lr_ratio)
            all_params.extend(params)
            lr_descriptions.extend(["text_encoder" + (" " + d if d else "") for d in descriptions])
...
def create_arch_network_from_weights(
    ...
    text_encoder_lora_num=1,
    unet_lora_num=2,
    **kwargs,
) -> LoRANetwork:
    return create_network_from_weights(
        HUNYUAN_TARGET_REPLACE_MODULES, multiplier, weights_sd, text_encoders, unet, for_inference,text_encoder_lora_num=text_encoder_lora_num,unet_lora_num=unet_lora_num, **kwargs
    )
